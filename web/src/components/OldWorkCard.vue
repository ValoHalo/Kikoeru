  ...
  <q-card class="card">
    ...
    <div v-if="!thumbnailMode">
      ...
      <div v-show="metadata.title" class="row items-center">
        ...
        <div class="col-auto q-ml-sm">
          ...
          <q-tooltip v-if=metadata.rate_count_detail class="text-subtitle1">
            <div>{{ $t('oldWorkCard.average', { rate_average_2dp: metadata.rate_average_2dp }) }}</div>
            <div v-for="(rate, index) in sortedRatings" :key=index class="row items-center">
              <div class="col">{{ $t('oldWorkCard.stars', { count: rate.review_point }) }}</div>
      ...
      <div v-show="metadata.title">
        <span class="q-mx-sm text-weight-medium text-h6 text-red">{{ $t('oldWorkCard.price', { price: metadata.price }) }}</span>
        <span>{{ $t('oldWorkCard.sales', { count: metadata.dl_count }) }}</span>
        <span v-if="metadata.nsfw === false" class="q-mx-sm" style="background: #e6f7d6; color: #56842a">{{ $t('oldWorkCard.allAges') }}</span>
      </div>
